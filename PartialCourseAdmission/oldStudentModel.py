...
class oldStudentsModel():
    
    def __init__(self):
        self.cnx = connectionUtility.getConnection()
        
    def extract_data_from_database(self):
        sql = ("Select id, LastName, FirstName, MI, IDNumber, Course, Program, Year from tbl_oldstudents")
        self.cursor.execute(sql)
        result = self.cursor.fetchall()
        return result
    
    def read_data(self):
        sql = ("Select id from tbl_oldstudents where id = (select MAX(id) from tbl_oldstudents") 
        self.cursor.execute(sql)
        result = self.cursor.fetchall()
        return result
    ...
    def insertStudentData(self):
        ...
        infile = ('D:/DOCU/Partial_Course_Admission_System/PartialCourseAdmission/oldStudentsInfo.csv') # <-- NOTE: Set & edit folder path here from your D: drive where this project folder is placed.
        from sqlalchemy import create_engine
        
        df = pd.read_csv(infile,na_values = " NaN", delimiter = ',', encoding = 'latin-1')
        df['id'] = pd.read_sql_query('select ifnull(MAX(id),0)+1 from tbl_oldstudents',self.cnx).iloc[0,0]+range(len(df))
        engine = create_engine('mysql+mysqlconnector://{0}:{1}@{2}/{3}'.format(user,password,host,db),pool_recycle = 1,pool_timeout = 57600).connect()
        
        df.to_sql(con = engine, name = 'tbl_oldstudents', if_exists = 'append', index = False, chunksize = 1)
        return
